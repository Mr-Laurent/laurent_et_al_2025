...
###------------------------------------### 
#### Function for the porportion plot ####
###------------------------------------### 
...
TL_prop_plot_gut<-function(celtypannot,df3){
  ...
  for(i in 1:length(listobj)){
    ...
    # A AJOUTER POUR SUBSET SUR PATIETNS PRESENTS 
    # T_df3@df3hcco[which(T_df3@df3hcco$sample%in%common_spl),]
  ...
  theme_classic()+labs( x = ' ', y = 'Proportion',fill = 'Celltype')+ 
    ...
    dfannot<-data.frame(patient=levels(as.factor(df4$sample)),origin=df4[match(levels(as.factor(df4$sample)), df4$sample), "location"],
                        ...
                        # status=df4[match(levels(as.factor(df4$sample)), df4$sample), "status"],
  ...
  plot3<-((pt3_1 / pt3_2) / pt3_3) / pt3_4 + 
    plot_layout(widths = c(2, 1), nrow=4, heights = c(6,1.2,1.2,1.2))
  pt4_1<-pt4+ggtitle("CD")+noyaxe   #+guides(fill= "none") pour Mono/Mac ratio j'enleve les UC donc besoin légende ici
...
# pdf("./Figure 1/Fig1F_Prop.pdf",width = 8,height = 6)
# plot$plot2+plot$plot3+plot$plot4+plot_layout(design = plot$design_perso) 
# dev.off()
pdf("./Figure 1/Fig1F_Propv2.pdf",width = 8,height = 6)
plot_smol$pt2_1+ylab('Proportion in total mo-macs')+ theme(axis.text.x = element_text(size=rel(1.2)),
                      axis.text.y = element_text(size=rel(1.2)),
                      axis.title.y = element_text(size=rel(2.2)))+
  plot_smol$pt3_1+theme(axis.text.x = element_text(size=rel(1.2)))+
  plot_smol$pt4_1+ theme(axis.text.x = element_text(size=rel(1.2)))+
  plot_layout(design = plot_smol$design_perso) 
dev.off()




# Figure 1 G: proportions as boxplots:
...
palette<-c("#ABCBD6","#C97185","#C22D4F")
plot6<-ggplot(df4, aes(x =category, y =Freq, color =category, fill=category))+
  geom_boxplot(lwd=2, color=palette, fill=palette, alpha=0.5)+scale_x_discrete(limits = c("HC ileum","CD uninv.", "CD"))+
  ...
  stat_compare_means(label.y = max(df4$Freq)+0.35,size = 8) # size is for K-W pvalue title 


pdf("./Figure 1/Fig1G_BoxP.pdf",width = 6,height = 10)
plot6
dev.off()
